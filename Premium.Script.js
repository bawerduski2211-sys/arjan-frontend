...
// --- ٢. فرمان بۆ گوهۆڕینا فۆرمان (Toggle Forms) ---
...
// --- ٣. فرمان بۆ پاسۆردێ ژبیرکری (Forgot Password) ---
async function handleForgotPassword() {
    const email = document.getElementById('log-email').value;
    if (!email) { 
        alert("هیڤییە ئیمەیڵێ خۆ ل خانەیا لۆگینێ بنڤێسە دا لینکێ تە بۆ بنێرین 📧"); 
        return; 
    }
    try {
        const { error } = await supabase.auth.resetPasswordForEmail(email);
        if (error) throw error;
        alert("لینکێ نووکرنا پاسۆردی بۆ ئیمەیڵا تە هات 📩");
    } catch (error) {
...
// --- ٦. پشکنینا بکارئینەری (User Status) ---
